...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const limit = parseBoundedInteger(new URL(req.url).searchParams.get("limit"), 5, 1, 5);
    ...
    for (const rawItem of rawItems || []) {
      try {
        ...
        const baseSlug = slugify(draft.title_vi);
        const datePrefix = rawItem.published_at ? rawItem.published_at.slice(0, 10) : new Date().toISOString().slice(0, 10);
        ...
        result.push({ raw_item_id: rawItem.id, slug });
      } catch (error) {
